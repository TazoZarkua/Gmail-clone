...
section_select_1.addEventListener('click', () => {
    ...
    selectForm.addEventListener('submit', (event) => {
        ...
        const searchInput = document.getElementById('search-input');
        const key = searchInput.value.trim().toLowerCase();
        let filtered = messages.filter(message => message.author.trim().toLowerCase().includes(key) || message.message.trim().toLowerCase().includes(key));
        if (filtered) {
            ...
            document.querySelector('.chat-inbox').innerHTML = searchHTML;
        }
    })
});
...
section_select_2.addEventListener('click', () => {
    ...
    selectForm.addEventListener('submit', (event) => {
        ...
        const searchInput = document.getElementById('search-input');
        const key = searchInput.value.trim().toLowerCase();
        const searchedMessage = promoMessage.filter(message => message.author.trim().toLowerCase().includes(key) || message.message.trim().toLowerCase().includes(key));
        if (searchedMessage) {
            searchedMessage.forEach((message) => {
                searchHTML += `
                            <div class="message-line">
   ...
   `
    ...
    selectForm.addEventListener('submit', (event) => {
        ...
        <div class="message-line">
            ...
            document.querySelector('.chat-inbox').innerHTML = filteredHTML;
        }
    })
});
